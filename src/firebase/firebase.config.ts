...
export const firebase = () => {
  const firebaseConfig = {
    apiKey: "AIzaSyDwYFtP6WK3TiaNSL-yzxguc05KtCcYweg",
    authDomain: "note-taking-app-5a43a.firebaseapp.com",
    projectId: "note-taking-app-5a43a",
    storageBucket: "note-taking-app-5a43a.firebasestorage.app",
    messagingSenderId: "971849941633",
    appId: "1:971849941633:web:14d4dd2f7296ad96af8380",
  };
  // Initialize Firebase
  ...
  const signIn = (eml: string, pass: string) => {
    let user = signInWithEmailAndPassword(auth, eml, pass)
      .then((userCred) => {
        return userCred;
      })
      .catch((err) => {
  ...
  function createUser(eml: string, pass: string) {
    let user = createUserWithEmailAndPassword(auth, eml, pass)
      .then((userCred) => {
        return userCred;
      })
      .then((user) => {
